...
fn compute_polygon_vertex(triple: &[BackendCoord;3], d: f64) -> BackendCoord {
    ...
    let a_p = (triple[1].0 as f64 + d * a_n.0, triple[1].1 as f64 + d * a_n.1);
    let b_p = (triple[1].0 as f64 + d * b_n.0, triple[1].1 as f64 + d * b_n.1);
...
fn traverse_vertices<'a>(mut vertices: impl Iterator<Item = &'a BackendCoord>, width: u32, mut op: impl FnMut(BackendCoord)) {
    ...
    op(((a.0 as f64 + n.0 * width as f64 / 2.0).round() as i32,
