...
impl <'a, X: Clone, Y:PartialOrd> CandleStick<'a, X, Y> {
    ...
    pub fn new<GS:Into<ShapeStyle<'a>>, LS:Into<ShapeStyle<'a>>>(x: X, open:Y, high:Y, low:Y, close:Y, gain_style:GS, loss_style:GS, width: u32) -> Self {
        return Self {
            ...
            width,
            points: [(x.clone(), open), (x.clone(), high), (x.clone(), low), (x.clone(), close)]
        };
    }
}
...
impl <'a, X:'a, Y:'a + PartialOrd> Drawable for CandleStick<'a, X, Y> {
    fn draw<DB: DrawingBackend, I: Iterator<Item = BackendCoord>>(
        ...
        if points.len() == 4 {
            ...
            let (l,r) = (self.width as i32 / 2, self.width as i32 - self.width as i32 / 2);
            points[0].0 -= l;
            points[3].0 += r;

            backend.draw_line(points[1], points[2], &Box::new(self.style.color))?;
            backend.draw_line((points[1].0 - l, points[1].1), (points[1].0 + r, points[1].1), &Box::new(self.style.color))?;
            backend.draw_line((points[2].0 - l, points[2].1), (points[2].0 + r, points[2].1), &Box::new(self.style.color))?;
