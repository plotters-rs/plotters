...
///
use rusttype::{Font, Scale, point, Error};
...
fn load_font_data(face:&str) -> FontResult<&'static [u8]> {
    match FONT_DATA_CACHE.lock().map(|mut cache| {
        ...
        return Ok(unsafe{std::mem::transmute::<_, &'static [u8]>(&cache.get(face).unwrap()[..])});
    }) {
        Ok(what) => what, 
...
impl <'a> FontDesc<'a> {
    ...
    pub fn draw<E, DrawFunc:FnMut(u32,u32,f32)->Result<(),E>>(&self, text: &str, (x,y):(u32,u32), mut draw:DrawFunc) -> FontResult<Result<(),E>> {
